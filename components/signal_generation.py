...
def handle_signal_generation():
    files = st.file_uploader(
        "Choose a file", accept_multiple_files=True, type=["xlsx"], key=2
    )
        ...
        with st.chat_message("assistant", avatar=ASSISTANT_AVATAR):
            ...
            with get_openai_callback() as cb:
                ...
                full_response = conversation(
                    {
                        "question": f"{prompt} , these are some signals for customers that should serve as an example : {signals}. makes sure that you use the same format , without any explanations . dont include the signals that i listed"
                    }
                )["text"]
                t2 = perf_counter()
            ...
            st.session_state["log"].append(
                (prompt, "Signal_Generator", total_cost, total_tokens, t2 - t1)
            )
