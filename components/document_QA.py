                ...
                if check_csv_files(st.session_state.uploaded_files) == True:
                    if len(st.session_state.uploaded_files) > 0:
                        ...
                        try:
                            ...
                            full_response = csv_conversation(
                                {"question": f' given this list of CSVs  \n :{dataframes[0:min(3, len(st.session_state.uploaded_files))]} ,{prompt} '})[
                                'text']
                    ...
                    logger.info('Task completed',
                                extra={"custom_dimensions": {'TaskType': 'Document_Q&A', 'Price': f'${total_cost:.3f}', 'Tokens': f'{total_tokens:.3f}', 'Time': f'{t2 - t1:.3f}'})
                    st.session_state.chat_his.append((prompt, full_response))
                    st.markdown(full_response)
